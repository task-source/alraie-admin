...
const AboutUs: React.FC = () => {
    ...
    (async () => {
      ...
      try {
        const results = await Promise.allSettled([
          api.get("/aboutUs?lang=en"),
          api.get("/aboutUs?lang=ar"),
        ]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await api.post("/aboutUs/update", { html, language: lang });
      setEditing(false);
      editor?.setEditable(false);

      if (res.data.success) {
        showAlert("success", "About Us updated successfully");
      }
    } catch (err) {
      showApiError(err);
    } finally {
