...
const PrivacyPolicy: React.FC = () => {
  ...
  // 🔹 Fetch both EN + AR policy initially
  useEffect(() => {
    (async () => {
      showLoader();
      try {
        ...
        console.error('Failed to fetch one or more privacy policies', err);
      } finally {
        hideLoader();
      }
    })();
  ...
  // 🔹 Update editor content when switching language
  useEffect(() => {
    if (editor) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const html = lang === 'en' ? contentEn : contentAr;
      await api.post('/privacyPolicy/update', { html, language: lang });
      setEditing(false);
